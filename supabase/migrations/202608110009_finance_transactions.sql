...
create index if not exists finance_transactions_user_updated_idx
  on public.finance_transactions(user_id, updated_at);
create index if not exists finance_transactions_user_time_idx
  on public.finance_transactions(user_id, transacted_at);

alter table public.finance_transactions enable row level security;

drop policy if exists finance_transactions_owner_policy
...
begin
  ...
  entity_kind := case tg_table_name
    ...
    when 'finance_transactions' then 'finance_transaction'
...
drop trigger if exists zhixu_revision_trigger
  on public.finance_transactions;
...
as $$
  select jsonb_build_object(
    ...
    'entities', jsonb_build_object(
      ...
      'finance_transaction', coalesce((select jsonb_agg(to_jsonb(t) - 'user_id') from public.finance_transactions t where t.user_id = auth.uid()), '[]'::jsonb)
    )
  );
$$;

revoke all on table public.finance_transactions from public;
